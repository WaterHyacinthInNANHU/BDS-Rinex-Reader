...
/* single-point positioning ----------------------------------------------------单点定位（顶层函数）
...
*          double *azel     IO  azimuth/elevation angle (rad) (NULL: no output)方位角/仰角
                  ...
                  char *msg)
{
    ...
    satposs(sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh);//按照所观测到的卫星顺序计算出每颗卫星的位置、速度、{钟差、频漂}
    
    /* estimate receiver position with pseudorange */
    stat=estpos(obs,n,rs,dts,var,svh,nav,&opt_,sol,azel_,vsat,resp,msg);//通过伪距实现绝对定位，计算出接收机的位置和钟差，顺带返回实现定位后每颗卫星的{方位角、仰角}、定位时有效性、定位后伪距残差
